...
using UserManagement.Application.Queries.Roles.FetchRoleByTitle;
...
namespace SchoolManagementAppApi.ApplicationService.MiddleWares
{
    public static class SeedDefaultRolesExtension
    {
        ...
        private static async Task PersistRegistrarRole(IEnumerable<Permission> permissions, IMediator mediator)
        {
            ...
            var roleId = await GetRoleId(createRoleCommand, mediator);
        ...
        private static async Task<Guid> GetRoleId(CreateRoleCommand createRoleCommand, IMediator mediator)
        {
            ...
            if (createRoleResponse.WasSuccessful) return createRoleResponse.Data.Id;

            var fetchedRole = await mediator.SendQueryAsync<Role, FetchRoleByTitleQuery, FetchRoleByTitleQueryHandler, RoleResponse>(new FetchRoleByTitleQuery { Title = createRoleCommand.Title });

            return fetchedRole.Data.Id;
        }
    }
}
